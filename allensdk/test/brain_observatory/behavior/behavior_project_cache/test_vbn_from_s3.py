...
from allensdk.brain_observatory.\
    behavior.behavior_project_cache.behavior_ecephys_project_cache \
    import VisualBehaviorEcephysProjectCache
...
def test_manifest_methods(tmpdir, vbn_s3_cloud_cache_data):
    ...
    create_bucket(bucket_name,
                  ...
                  data['data'],
                  data['metadata'])
    cache = VisualBehaviorEcephysProjectCache.from_s3_cache(cache_dir,
                                                            bucket_name,
                                                            project_name)
...
def test_local_cache_construction(
    ...
    cmd = 'VisualBehaviorEcephysProjectCache.construct_local_manifest()'
...
def test_load_out_of_date_manifest(
    ...
    """
    Test that VisualBehaviorEcephysProjectCache can load a
...
def test_when_data_updated(tmpdir, vbn_s3_cloud_cache_data, data_update):
    ...
    create_bucket(bucket_name,
                  ...
                  data['data'],
                  data['metadata'])
    cache = VisualBehaviorEcephysProjectCache.from_s3_cache(
        cache_dir,
        bucket_name,
        project_name)
    ...
    cmd = 'VisualBehaviorEcephysProjectCache.load_manifest'
    with pytest.warns(OutdatedManifestWarning, match=name3) as warnings:
        VisualBehaviorEcephysProjectCache.from_s3_cache(
            cache_dir,
            bucket_name,
            project_name)
...
def test_load_last(tmpdir, vbn_s3_cloud_cache_data, data_update):
    ...
    msg = 'VisualBehaviorEcephysProjectCache.compare_manifests'
